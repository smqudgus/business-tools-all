...
module.exports = async function handler(req,res){
  try{
    ...
    const {data:links,error:lErr}=await supabase.from('competitor_links').select('*').in('product_id',ids.length?ids:['00000000-0000-0000-0000-000000000000']);
    if(lErr) throw lErr;
    ...
    for(const product of products){
      ...
      for(const link of pLinks){
        ...
        try{
          const extracted=await fetchAndExtract(link.url,product);
          const row={product_id:product.id,seller_key:link.seller_key,url:link.url,price:extracted.price,confidence:extracted.confidence,note:extracted.note};
          ...
          await supabase.from('price_history').insert({product_id:product.id,seller_key:link.seller_key,url:link.url,price:null,confidence:'fail',note:e.message||String(e)});
          results.push({product:product.name,seller:link.seller_key,ok:false,note:e.message||String(e)});
        }
      }
    }

    return res.status(200).json({ok:true,successCount:results.filter(r=>r.ok).length,failCount:results.filter(r=>!r.ok).length,results});
